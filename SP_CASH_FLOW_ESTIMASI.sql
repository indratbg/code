...
IS
  ...
  CURSOR CSR_RETAIL
  IS
    SELECT KATEGORI,SUM(AMT)AMT, SUM(DECODE(SIGN(AMT),1,DECODE(SIGN(AMT-SALDO_RDI),-1,AMT,SALDO_RDI ),0)) EST_RDI,
      ...
      (
        ...
        FROM
          (
            ...
            AND (M.CLIENT_TYPE_3 = V_CLIENT_TYPE_3
            OR V_CLIENT_TYPE_3   ='%')
            AND CONTR_STAT      <>'C'
            GROUP BY T.CLIENT_CD
          --BIAYA BIAYA
          UNION ALL
          SELECT F_GET_CASHFLOW_CATEGORY(client_cd)kategori,CLIENT_CD,
          SUM(DECODE(DB_CR_FLG,'D',CURR_VAL,-CURR_VAL))NET_AMT
    ...
    LOOP
      ...
      BEGIN
        ...
        INTO TMP_CASH_FLOW_ESTIMASI
          ...
          VALUES
          (
            V_KATEGORI,DECODE(REC.KATEGORI,'IR',DECODE(SIGN(REC.AMT),1,REC.AMT,0),REC.EST_RDI), 0, P_RAND_VALUE,P_USER_ID
          );
      EXCEPTION
      WHEN OTHERS THEN
        V_ERROR_CODE:=-10;
      ...
      END IF;
          ...
          BEGIN
            ...
            INTO TMP_CASH_FLOW_ESTIMASI
              ...
              VALUES
              (
                V_KATEGORI,0,ABS(REC.AMT),P_RAND_VALUE,P_USER_ID
              );
          EXCEPTION
          WHEN OTHERS THEN
            V_ERROR_CODE:=-10;
    ...
    BEGIN
      ...
      FROM
        (
          ...
          FROM
            (
              SELECT 'F' KATEGORI,TO_CHAR(TRX_SEQ_NO)TRX_SEQ_NO,SUM(DECODE(TRX_TYPE,'S',NET_AMOUNT,-NET_AMOUNT))NET_AMT
              FROM T_BOND_TRX
