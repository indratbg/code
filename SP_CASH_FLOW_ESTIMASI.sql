      ...
      (
        ...
        FROM
          ...
          FROM T_ACCOUNT_LEDGER A, MST_CLIENT M, (
            ...
            )TEMP, (SELECT GL_A FROM MST_GLA_TRX WHERE JUR_TYPE='REPO')G
          ...
          UNION ALL
          select F_GET_CASHFLOW_CATEGORY(sl_acct_cd)kategori, sl_acct_cd, sum(net_amt)net_amt 
          from 
          (
              ...
              AND TRIM(A.GL_ACCT_CD)=G.GL_A(+)
              AND G.GL_A IS NULL
              --AND TRIM(A.GL_ACCT_cD) NOT IN ('1415','2415')--15/01/2018[INDRA] SUPAYA REPO TIDAK IKUT
              AND M.CUSTODIAN_CD IS NULL--tidak custodian
              UNION ALL
              SELECT sl_acct_cd, NVL(b.deb_obal,0)-NVL(b.cre_obal,0) beg_bal
              FROM t_day_trs b, MST_CLIENT M, (SELECT GL_A FROM MST_GLA_TRX WHERE JUR_TYPE='REPO')G
    ...
    FROM
      ...
      FROM
        (
          ...
          --BIAYA BIAYA
          UNION ALL
          SELECT 'IR' KATEGORI, A.SL_ACCT_CD, SUM(DECODE(DB_CR_FLG,'D',CURR_VAL,-CURR_VAL))NET_AMT
          FROM T_ACCOUNT_LEDGER A, MST_CLIENT M, (
              SELECT XN_DOC_NUM
              FROM TEMP_DAILY_CASH_FLOW
              WHERE RAND_VALUE=P_RAND_VALUE
          ...
          --OUTSTANDING
          UNION ALL
           SELECT 'IR' KATEGORI,sl_acct_cd, sum(net_amt) 
           from(
          SELECT  A.SL_ACCT_CD, DECODE(A.DB_CR_FLG,'D',CURR_VAL,-CURR_VAL) NET_AMT
          FROM t_account_ledger a, mst_client m, (SELECT GL_A FROM MST_GLA_TRX WHERE JUR_TYPE='REPO')G
          WHERE a.SL_ACCT_CD=m.client_cd
          AND TRIM(A.GL_ACCT_CD) = G.GL_A(+)
          AND G.GL_A IS NULL
          AND a.doc_date BETWEEN V_BEG_BAL_DATE AND P_DUE_DATE
          ...
          AND CLIENT_TYPE_3<>'M'
          UNION ALL
          SELECT B.SL_ACCT_CD, b.deb_obal-b.cre_obal beg_bal
          FROM t_day_trs b, MST_CLIENT M,  (SELECT GL_A FROM MST_GLA_TRX WHERE JUR_TYPE='REPO')G
          WHERE  B.SL_ACCT_CD             =M.CLIENT_CD
          AND TRIM(B.GL_ACCT_CD)=G.GL_A(+)
          AND G.GL_A IS NULL
