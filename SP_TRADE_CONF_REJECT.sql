create or replace 
PROCEDURE "SP_TRADE_CONF_REJECT" (
...
BEGIN
	...
	BEGIN
		DELETE FROM INSISTPRO_RPT.LAP_TRADE_CONF WHERE UPDATE_DATE = P_UPDATE_DATE AND UPDATE_sEQ=P_UPDATE_SEQ;
	EXCEPTION
			WHEN OTHERS THEN
				v_error_code := -10;
	...
	BEGIN
		DELETE FROM T_TC_DOC WHERE 	UPDATE_DATE = P_UPDATE_DATE AND UPDATE_sEQ=P_UPDATE_SEQ;
	EXCEPTION
			WHEN OTHERS THEN
				v_error_code := -20;
				v_error_msg :=  SUBSTR('UPDATE T_TC_DOC '||SQLERRM,1,200);
