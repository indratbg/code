create or replace PROCEDURE SP_LST_TYPE2_UPD(
    P_SEARCH_CL_TYPE2 LST_TYPE2.CL_TYPE2%TYPE,
    P_CL_TYPE2   LST_TYPE2.CL_TYPE2%TYPE,
    P_CL_DESC    LST_TYPE2.CL_DESC%TYPE,
    P_CRE_DT   LST_TYPE2.CRE_DT%TYPE,
    P_USER_ID    LST_TYPE2.USER_ID%TYPE,
    P_UPD_DT   LST_TYPE2.UPD_DT%TYPE,
    P_UPD_BY   LST_TYPE2.UPD_BY%TYPE,
...
IS
  ...
  v_table_name T_TEMP_HEADER.table_name%TYPE := 'LST_TYPE2';
  ...
  CURSOR csr_table
  IS
    SELECT * FROM LST_TYPE2 WHERE CL_TYPE2 = P_SEARCH_CL_TYPE2;
  v_temp_detail Types.temp_detail_rc;
  v_rec LST_TYPE2%ROWTYPE;
  v_cnt         INTEGER;
  i             INTEGER;
  V_FIELD_CNT   NUMBER;
...
BEGIN
      ...
      BEGIN
        ...
        INTO v_table_rowid
        FROM LST_TYPE2
        WHERE CL_TYPE2 = P_SEARCH_CL_TYPE2;
      EXCEPTION
      WHEN NO_DATA_FOUND THEN
        v_table_rowid := NULL;
      WHEN OTHERS THEN
        v_error_code := -2;
        v_error_msg  := SUBSTR('Retrieve  '|| v_table_name||' for '||P_SEARCH_CL_TYPE2||SQLERRM,1,200);
        RAISE v_err;
      END;
      ...
      IF v_table_rowid IS NULL THEN
        ...
        BEGIN
          ...
          FROM
            (
              SELECT MAX(CL_TYPE2) CL_TYPE2
              FROM
                (
                  SELECT DECODE (field_name, 'CL_TYPE2', field_value, NULL) CL_TYPE2
