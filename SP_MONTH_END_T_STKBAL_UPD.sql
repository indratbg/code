create or replace 
PROCEDURE SP_MONTH_END_T_STKBAL_UPD(
	P_BAL_DT		LAP_MKBD_VD51.MKBD_DATE%TYPE,
	...
	v_table_name 				IPNEXTG.T_MANY_DETAIL.table_name%TYPE := 'T_STKBAL';
...
BEGIN
	...
	OPEN v_MANY_DETAIL FOR
		...
		( 
			SELECT  'BAL_DT'  AS field_name, TO_CHAR(P_BAL_DT,'yyyy/mm/dd hh24:mi:ss')  AS field_value, DECODE(v_rec.REPORT_DATE, P_BAL_DT,'N','Y') upd_flg FROM dual
	...
	END IF;
		...
		BEGIN
		SP_MONTH_END_T_SECU_BAL_UPD(P_BAL_DT,
									P_UPD_STATUS,
									P_UPDATE_DATE,
									P_UPDATE_SEQ,
									p_record_seq,
	...
	EXCEPTION
		WHEN OTHERS THEN
			v_error_code := -9;
			v_error_msg := SUBSTR('SP_MONTH_END_T_SECU_BAL_UPD '||v_table_name||SQLERRM,1,200);
			RAISE v_err;
	END;
	
	IF v_error_code < 0 THEN
	    v_error_code := -10;
		v_error_msg := 'SP_MONTH_END_T_SECU_BAL_UPD '||v_error_msg;
		RAISE v_err;
	END IF;
