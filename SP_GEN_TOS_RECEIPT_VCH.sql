create or replace PROCEDURE SP_GEN_TOS_RECEIPT_VCH(
    P_PAYMENT_DATE DATE,
    P_STK_CD       VARCHAR2,
...
--12FEB2018 ISI BRANCH CODE DI T_ACCOUNT_LEDGER
  CURSOR CSR_CLIENT
  IS
  ...
  V_ERR        EXCEPTION;
  V_MENU_NAME T_MANY_HEADER.MENU_NAME%TYPE:='GENERATE RECEIPT TENDER OFFER SELL';
...
BEGIN
  ...
  BEGIN
    SELECT DSTR1, DSTR2 INTO V_AB_GL_A, V_AB_SL_A
    FROM MST_SYS_PARAM
  ...
  BEGIN
    SELECT DFLG1 INTO V_CHECK_BRANCH
    FROM MST_SYS_PARAM
  ...
  BEGIN
    SELECT DSTR1, DSTR2, PARAM_CD3 INTO V_GL_ACCT_CD, V_SL_ACCT_CD, V_BRANCH
    FROM MST_SYS_PARAM
    WHERE param_id='VOUCHER TENDER OFFER'
    AND PARAM_CD1 ='GL ACCT'
    AND PARAM_CD2 ='BANK'
  ...
  LOOP--SETT FOLDER CD
    ...
    --12FEB2018 GET BRANCHCODE FROM MST_CLIENT
    BEGIN
      SELECT TRIM(BRANCH_CODE) INTO  V_BRANCH_CD
      FROM MST_CLIENT
      WHERE CLIENT_CD         = V_SL_ACCT_CD
      AND APPROVED_STAT='A';
    EXCEPTION
    WHEN OTHERS THEN
      V_ERROR_CODE := -215;
      V_ERROR_MSG  :=SUBSTR('SELECT BRANCH_CODE FROM MST_CLIENT '||' '||V_SL_ACCT_CD||' '||SQLERRM,1,200);
      RAISE V_ERR;
    END;
    --END 12FEB2018
