...
IS
  ...
  IS
    ...
    FROM
      (
        SELECT KATEGORI,  SUM(decode( sign( net_amt), 1, net_amt,0))  deb_amt,
                                 SUM(decode( sign( net_amt), -1, abs(net_amt),0))    cre_amt
        FROM ( 
                SELECT xn_doc_num, kategori, sum(net_amt) net_amt
                from(
                        SELECT a.xn_doc_num, 
                        decode(m.client_cd, null,F_GET_CASHFLOW_CATEGORY(trim(a.gl_acct_cd)),m.client_cd) client_cd,
                        decode(m.client_cd, null, F_GET_CASHFLOW_CATEGORY(trim(a.gl_acct_cd)),F_GET_CASHFLOW_CATEGORY(m.client_cd) ) AS kategori,
                        DECODE(a.db_cr_flg,'C', 1, -1) * a.curr_val AS net_amt
                        FROM
                          (
                            SELECT t.xn_doc_num, t.gl_acct_cd
                            FROM t_account_ledger t,
                              (SELECT XN_DOC_NUM FROM TEMP_DAILY_CASH_FLOW WHERE RAND_VALUE=V_RANDOM_VALUE AND USER_ID=P_USER_ID) TEMP
