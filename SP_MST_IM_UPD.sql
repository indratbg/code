create or replace PROCEDURE "SP_MST_IM_UPD" (
		P_SEARCH_IM_CODE		MST_IM.IM_CODE%TYPE,
		P_IM_CODE		MST_IM.IM_CODE%TYPE,
		P_IM_NAME		MST_IM.IM_NAME%TYPE,
		P_CRE_BY		MST_IM.CRE_BY%TYPE,
		P_CRE_DT		MST_IM.CRE_DT%TYPE,
		P_UPD_BY		MST_IM.UPD_BY%TYPE,
		P_UPD_DT		MST_IM.UPD_DT%TYPE,
...
v_table_name T_TEMP_HEADER.table_name%TYPE := 'MST_IM';
...
SELECT *
FROM MST_IM
WHERE IM_CODE = P_SEARCH_IM_CODE;

v_rec MST_IM%ROWTYPE;
...
BEGIN
		...
		IF 	  P_UPD_STATUS = 'I' AND P_SEARCH_IM_CODE <> P_IM_CODE THEN
			       v_error_code := -2001;
				   v_error_msg  := 'jika INSERT, P_SEARCH_IM_CODE harus sama dengan P_IM_CODE';
				   RAISE v_err;
			END IF;
			...
			WHEN OTHERS THEN
				     v_error_code := -2;
					 v_error_msg :=  SUBSTR('Retrieve  '|| v_table_name||' for '||P_SEARCH_IM_CODE||SQLERRM,1,200);
					 RAISE v_err;
				  END;

			IF 	  P_UPD_STATUS = 'I' AND v_table_rowid IS NOT NULL THEN
			       v_error_code := -2002;
				   v_error_msg  := 'DUPLICATED BROKER CODE';
				   RAISE v_err;
			END IF;
			
			IF 	  P_UPD_STATUS = 'U'   AND P_SEARCH_IM_CODE  <> P_IM_CODE THEN
				 BEGIN
	   	 		 SELECT COUNT(1) INTO v_cnt
				 FROM MST_IM
				 WHERE IM_CODE = P_IM_CODE;
				 EXCEPTION
				 WHEN NO_DATA_FOUND THEN
				 	  v_cnt := 0;
				WHEN OTHERS THEN
					     v_error_code := -3;
						 v_error_msg :=  SUBSTR('Retrieve  '|| v_table_name||' for '||P_IM_CODE||SQLERRM,1,200);
						 RAISE v_err;
				  END; 
			...
			IF v_table_rowid IS NULL THEN
					 ...
					 AND d.field_name = 'IM_CODE'
					 AND   d.field_value = P_SEARCH_IM_CODE
		...
		( 
			...
			UNION
			SELECT  'IM_NAME'  AS field_name, p_IM_NAME AS field_value, DECODE(trim(v_rec.IM_NAME), trim(p_IM_NAME),'N','Y') upd_flg FROM dual
			UNION
