create or replace 
PROCEDURE SPR_RECON_STK_BAL_MAIN_001(P_BGN_STK VARCHAR2,
											P_CURR VARCHAR2,
...
-- 
   ...
   v_random_value := abs(dbms_random.random);
    ...
    BEGIN
        SP_RPT_REMOVE_RAND('R_RECON_STK_BAL_MAIN_001',V_RANDOM_VALUE,V_ERROR_MSG,V_ERROR_CD);
    EXCEPTION
        WHEN OTHERS THEN
             V_ERROR_CD := -10;
	...
	SELECT  '001' AS sub_rek,  d.stk_cd, d.secu_theo, d.secu_onh, d.secu_os_buy, d.secu_os_sell, 							
	 ...
	 FROM(  SELECT    stk_cd, 							
          ...
          WHERE  ((d.secu_theo <> d.pdate_theo OR d.secu_theo <>  d.c_theo OR d.pdate_theo <> d.c_theo) 								
			  ...
			  AND ( (d.pdate_onh <> d.ksei AND P_ALL_RECORD = 'N') OR P_ALL_RECORD = 'Y'  );					
		
	 EXCEPTION
			WHEN OTHERS THEN
				 V_ERROR_CD := -30;
				 V_ERROR_MSG := SUBSTR('INSERT R_RECON_STK_BAL_MAIN_001 '||V_ERROR_MSG||SQLERRM(SQLCODE),1,200);
				RAISE V_err;
		END;
...
P_ERROR_MSG := '';
 ...
 EXCEPTION
  ...
  WHEN OTHERS THEN
   ...
   RAISE;
END SPR_RECON_STK_BAL_MAIN_001;