create or replace 
PROCEDURE         SP_FUND_BANK_MVMT_IMPORT (
...
/******************************************************************************
 ...
 21jan2016 dirubah utk CIMB, krn interest n tax tidak masuk ke t bank mutation
   krn bank reference blank, dibuatkan dummy bank ref dari timestamp+ trans type
******************************************************************************/
...
v_dummy_bank_ref  t_bank_mutation.BANKREFERENCE%TYPE; --21jan2016
...
--FOR rec  IN csr_data LOOP
  --  v_data := rec.data;
	...
	LOOP
        ...
        v_dummy_bank_ref := trim(v_str); --21jan2016
		END IF;
	...
	IF v_bankid = 'BNGA3' THEN
		...
		ELSE
         ...
         IF v_bankreference IS NULL THEN 
			   	  				 v_bankreference := v_dummy_bank_ref||v_cimb_tax_interest;
				   END IF;		
				   IF v_cimb_tax_interest = '198'  THEN
				   	  					  v_remark := 'Tax';
					ELSIF  v_cimb_tax_interest = '160' THEN 
					  					  v_remark := 'Interest';
					 END IF;	
         --21jan2016 end  
			 END IF;
