create or replace 
PROCEDURE Spr_T_Dhk(	P_SETTLE_DATE DATE,
                      P_OPTIONS VARCHAR2,
					  P_TYPE VARCHAR2,
                       P_USER_ID			VARCHAR2,
...
--7oct15, T contracts diganti T STK MOVEMENT					  
...
SELECT a.CLIENT_CD ,												
		...
		FROM( 		      SELECT client_cd, stk_Cd,										
			SUM(DECODE(SUBSTR(doc_num,5,1),'B',1,-1) * (total_share_qty + withdrawn_share_qty) ) net_qty		
			  FROM t_stk_movement
			  WHERE doc_dt BETWEEN v_trx_date AND  P_SETTLE_DATE
			  AND doc_stat = '2'
			  AND s_d_type = 'T'
			  AND due_dt_for_cert =  P_SETTLE_DATE
			  AND gl_acct_cd IN ('59','21')
			  GROUP BY client_Cd, stk_Cd		
				) a, 
				(
				 SELECT client_cd, DECODE(v_type, 'AB'	 ,'ANGGOTA BURSA', client_name) client_name, custodian_Cd, sid
				  FROM mst_client, v_broker_subrek
				  WHERE (client_cd = broker_client_cd AND v_type = 'AB')
				  OR (custodian_Cd IS NOT NULL AND  v_type = 'AB')
				  OR v_type = 'CLIENT' ) m,
				v_client_subrek14 b,								
			 v_broker_subrek c									
...
UNION ALL												
 SELECT  NULL client_cd, NULL client_name, sid, subrek001, subrek004, stk_cd, 0 ip_buy, 0 ip_sell, net_buy dhk_buy, net_sell dhk_sell												
  ...
  AND stk_Cd <> 'IDR'		
   AND (SUBSTR(subrek001,6,4) = '0000' AND v_type = 'AB') OR v_type <> 'AB'										
   ) d												
