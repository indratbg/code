create or replace 
PROCEDURE Sp_T_HAIRCUT_KOMITE_Upd(
	P_SEARCH_STATUS_DT T_HAIRCUT_KOMITE.STATUS_DT%TYPE,
	P_SEARCH_EFF_DT T_HAIRCUT_KOMITE.EFF_DT%TYPE,
	P_SEARCH_STK_CD T_HAIRCUT_KOMITE.STK_CD%TYPE,
	P_STATUS_DT		T_HAIRCUT_KOMITE.STATUS_DT%TYPE,
	P_STK_CD		T_HAIRCUT_KOMITE.STK_CD%TYPE,
	P_HAIRCUT		T_HAIRCUT_KOMITE.HAIRCUT%TYPE,
	P_CREATE_DT		T_HAIRCUT_KOMITE.CREATE_DT%TYPE,
	P_EFF_DT		T_HAIRCUT_KOMITE.EFF_DT%TYPE,
	P_UPD_DT		T_HAIRCUT_KOMITE.UPD_DT%TYPE,
	P_UPD_BY		T_HAIRCUT_KOMITE.UPD_BY%TYPE,
	P_USER_ID		T_HAIRCUT_KOMITE.USER_ID%TYPE,
	P_CRE_DT		T_HAIRCUT_KOMITE.CRE_DT%TYPE,
	...
	v_table_name 				T_TEMP_HEADER.table_name%TYPE := 'T_HAIRCUT_KOMITE';
...
WHERE EFF_DT = P_SEARCH_EFF_DT
	AND STK_CD = P_SEARCH_STK_CD
	AND	STATUS_DT = P_SEARCH_STATUS_DT;
v_temp_detail  Types.temp_detail_rc;

v_rec T_HAIRCUT_KOMITE%ROWTYPE;
...
BEGIN
			...
			IF 	  P_UPD_STATUS = 'I' AND (P_SEARCH_EFF_DT <> p_EFF_DT OR P_SEARCH_STATUS_DT <> p_STATUS_DT
								OR P_SEARCH_STK_CD <> P_STK_CD) THEN
			       v_error_code := -2001;
				   IF P_SEARCH_STATUS_DT <> p_STATUS_DT THEN
				   		v_error_msg := 'jika INSERT, P_SEARCH_STATUS_DT harus sama dengan P_STATUS_DT';
					END IF;
					IF P_SEARCH_EFF_DT <> p_EFF_DT THEN
				   		v_error_msg := 'jika INSERT, P_SEARCH_EFF_DT harus sama dengan P_EFF_DT';
					END IF;
			...
			END IF;
			 ...
			 FROM T_HAIRCUT_KOMITE
			 WHERE STATUS_DT = P_SEARCH_STATUS_DT AND EFF_DT = P_SEARCH_EFF_DT
				AND STK_CD = P_SEARCH_STK_CD;
			 EXCEPTION
			...
			WHEN OTHERS THEN
				     v_error_code := -2;
					 v_error_msg :=  SUBSTR('Retrieve  '|| v_table_name||' for '||P_SEARCH_STATUS_DT||SQLERRM,1,200);
					 RAISE v_err;
				  END;

			IF 	  P_UPD_STATUS = 'I' AND v_table_rowid IS NOT NULL THEN
			       v_error_code := -2001;
				   v_error_msg  := 'DUPLICATED STATUS DATE, EFFECTIVE DATE AND STOCK CD';
				   RAISE v_err;
			END IF;
			
			IF 	  P_UPD_STATUS = 'U' AND (P_SEARCH_STATUS_DT <> P_STATUS_DT OR P_SEARCH_EFF_DT <> P_EFF_DT
								OR P_SEARCH_STK_CD<> P_STK_CD) THEN
				 BEGIN
	   	 		 SELECT COUNT(1) INTO v_cnt
				 FROM T_HAIRCUT_KOMITE
				 WHERE  STATUS_DT = p_STATUS_DT AND
				EFF_DT = p_EFF_DT
					AND STK_CD = P_STK_CD;
				 EXCEPTION
				 WHEN NO_DATA_FOUND THEN
			...
			IF v_table_rowid IS NULL THEN
					 ...
					 FROM (SELECT MAX(STATUS_DT) STATUS_DT, MAX(EFF_DT) EFF_DT, MAX(STK_CD) STK_CD
							FROM (SELECT DECODE (field_name, 'STATUS_DT', field_value, NULL) STATUS_DT,
								DECODE (field_name, 'EFF_DT', field_value, NULL) EFF_DT,
								DECODE	(field_name, 'STK_CD', field_value, NULL) STK_CD
								 ...
								 AND (d.field_name = 'STATUS_DT' OR d.field_name = 'EFF_DT' OR d.field_name = 'STK_CD') 
								 AND h.APPROVED_status = 'E'))
					 WHERE STATUS_DT = P_SEARCH_STATUS_DT
						AND EFF_DT = P_SEARCH_EFF_DT
						AND STK_CD = P_SEARCH_STK_CD;
					 EXCEPTION
					 WHEN NO_DATA_FOUND THEN
					         v_pending_cnt := 0;
		...
		UNION
		SELECT  'HAIRCUT'  AS field_name, TO_CHAR(p_HAIRCUT)  AS field_value, DECODE(v_rec.HAIRCUT, p_HAIRCUT,'N','Y') upd_flg FROM dual
		UNION
