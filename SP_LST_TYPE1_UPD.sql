create or replace PROCEDURE SP_LST_TYPE1_UPD(
    P_SEARCH_CL_TYPE1 LST_TYPE1.CL_TYPE1%TYPE,
    P_CL_TYPE1 LST_TYPE1.CL_TYPE1%TYPE,
    P_CL_DESC LST_TYPE1.CL_DESC%TYPE,
    P_CRE_DT LST_TYPE1.CRE_DT%TYPE,
    P_USER_ID LST_TYPE1.USER_ID%TYPE,
    P_UPD_DT LST_TYPE1.UPD_DT%TYPE,
    P_UPD_BY LST_TYPE1.UPD_BY%TYPE,
...
IS
  ...
  v_table_name T_TEMP_HEADER.table_name%TYPE := 'LST_TYPE1';
  ...
  CURSOR csr_table
  IS
    SELECT * FROM LST_TYPE1 WHERE CL_TYPE1 = P_SEARCH_CL_TYPE1;
  v_temp_detail Types.temp_detail_rc;
  v_rec LST_TYPE1%ROWTYPE;
  v_cnt         INTEGER;
  i             INTEGER;
  V_FIELD_CNT   NUMBER;
...
BEGIN
      ...
      WHEN OTHERS THEN
        v_error_code := -2;
        v_error_msg  := SUBSTR('Retrieve  '|| v_table_name||' for '||P_SEARCH_CL_TYPE1||SQLERRM,1,200);
        RAISE v_err;
      END;
      ...
      IF v_table_rowid IS NULL THEN
        ...
        BEGIN
          ...
          FROM
            (
              SELECT MAX(CL_TYPE1) CL_TYPE1
              FROM
                (
                  SELECT DECODE (field_name, 'CL_TYPE1', field_value, NULL) CL_TYPE1
      ...
      a, (
        SELECT 'CL_TYPE1' AS field_name, p_CL_TYPE1 AS field_value, DECODE(trim(v_rec.CL_TYPE1), trim(p_CL_TYPE1),'N','Y') upd_flg
        FROM dual
        UNION
        SELECT 'CL_DESC' AS field_name, p_CL_DESC AS field_value, DECODE(trim(v_rec.CL_DESC), trim(p_CL_DESC),'N','Y') upd_flg
        FROM dual
        UNION
