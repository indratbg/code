...
BEGIN	
		...
		BEGIN
			...
			VALUES (v_doc_num , p_doc_dt, p_coy_client_cd,
			   p_reks_cd, p_trx_type,DECODE(p_trx_type,'SUBS',DECODE(v_cnt,1,p_debit,0),DECODE(v_cnt,1,p_credit,0)),
			   DECODE(p_trx_type,'REDM',DECODE(v_cnt,2,p_credit,0),DECODE(v_cnt,2,p_debit,0)), DECODE(p_trx_type,'SUBS','Subscribe','Redeem'),'2',
	   ...
	   FOR v_seqno IN 1..2 LOOP
		...
		BEGIN
					INSERT INTO IPNEXTG.T_REKS_MOVEMENT (
							   ...
							   APPROVED_BY, APPROVED_STS)
			VALUES (v_doc_num , p_doc_dt, p_coy_client_cd,
			   p_reks_cd, v_trx_type, DECODE(v_trx_type,'RSUB',DECODE(v_cnt,1,p_debit,0),DECODE(v_cnt,1,p_credit,0)),
			    DECODE(v_trx_type,'RRED',DECODE(v_cnt,2,p_credit,0),DECODE(v_cnt,2,p_debit,0)), DECODE(p_trx_type,'SUBS','Reversal Subscribe',' Reversal Redeem'),'2',
			    v_gl_acct_cd, DECODE(v_cnt,1,'D','C'), v_cnt,
			    p_nab_unit, SYSDATE,p_approved_user_id,
			    NULL, NULL, NULL,
			    NULL,'A');
