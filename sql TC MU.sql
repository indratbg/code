SELECT c.client_name, NVL(c.contact_pers,c.client_name) contact_pers, 
       ...
       r.brch_name, r.brch_acct_num, r.phone_num brch_phone, r.fax_num brch_fax, 
       r.def_addr_1 brch_addr_1, r.def_addr_2 brch_addr_2, r.def_addr_3 brch_addr_3, 
	    s.rem_name, b.bank_name, b.short_bank_name,m.nama_prsh, m.no_ijin1, 
		d.tc_id,	 f.npwp_no,  f.sid, v.subrek001, F_BANK_ACCT_MASK(a.bank_acct_num,k.acct_mask) bank_acct_num,
		a.acct_name, nvl(a.bank_short_name,'') as bank_rdi,
	   'RG' AS mrkt_t3, 
	   'NG' AS mrkt_t2, 
	   'NG' AS mrkt_t1, 
	   'TN' AS mrkt_t0, 
...
FROM( SELECT  a.contr_dt,a.r_i, a.client_cd, a.beli_jual, a.stk_cd, a.status, a.lot_size, a.qty, 
				...
				a.pph_perc, a.mrkt_type, a.mrkt_txt, 
	...
	FROM( 	SELECT  contr_dt, SUBSTR(contr_num,6,1) R_I, 
				...
				FROM T_CONTRACTS 
				WHERE contr_dt BETWEEN  :d_bgn_date AND  :d_end_date 
				AND contr_stat <> 'C' 
				AND client_cd BETWEEN :s_beg_client AND :s_end_client 
				AND brch_cd   BETWEEN :s_beg_branch AND :s_end_branch 
				AND rem_cd    BETWEEN :s_beg_rem    AND :s_end_rem	) a, 
		...
		WHERE SUBSTR(xn_doc_num,8,3) = 'MFE' 
		  AND sl_acct_cd BETWEEN :s_beg_client AND :s_end_client 
		AND doc_date BETWEEN  :d_bgn_date AND  :d_end_date 
		AND (SUBSTR(gl_acct_cd,1,2) = '14' OR SUBSTR(gl_acct_cd,1,2) = '24')) B1 
		WHERE t.doc_date BETWEEN  :d_bgn_date AND  :d_end_date 
		and t.sl_acct_cd BETWEEN :s_beg_client AND :s_end_client
		...
		WHERE  a.contr_dt = minfee.doc_date(+) 
		 ...
		 (  SELECT CLient_cd, tc_date, TC_ID||decode(tc_rev,0,'',' rev.'||to_char(tc_rev)) tc_id
		     FROM T_TC_DOC
			 WHERE tc_date BETWEEN  :d_bgn_date AND  :d_end_date 
				AND client_cd BETWEEN :s_beg_client AND :s_end_client
            AND tc_status = 0 ) D,
