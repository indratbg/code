create or replace PROCEDURE Sp_FOMD_GROUP_LIMIT_Upd(
	P_SEARCH_GROUP_CD		FOMD_GROUP_LIMIT.GROUP_CD%TYPE,
	P_SEARCH_USERID		FOMD_GROUP_LIMIT.USERID%TYPE,
	P_SEARCH_CLIENT_CD		FOMD_GROUP_LIMIT.CLIENT_CD%TYPE,
	P_GROUP_CD		FOMD_GROUP_LIMIT.GROUP_CD%TYPE,
	P_USERID		FOMD_GROUP_LIMIT.USERID%TYPE,
	P_CLIENT_CD		FOMD_GROUP_LIMIT.CLIENT_CD%TYPE,
	P_CLIENT_TYPE		FOMD_GROUP_LIMIT.CLIENT_TYPE%TYPE,
	P_CRE_BY		FOMD_GROUP_LIMIT.CRE_BY%TYPE,
	P_CRE_DT		FOMD_GROUP_LIMIT.CRE_DT%TYPE,
	P_UPD_BY		FOMD_GROUP_LIMIT.UPD_BY%TYPE,
	P_UPD_DT		FOMD_GROUP_LIMIT.UPD_DT%TYPE,
...
v_table_name 					T_MANY_DETAIL.table_name%TYPE := 'FOMD_GROUP_LIMIT';
...
v_rec FOMD_GROUP_LIMIT%ROWTYPE;
...
BEGIN
	IF P_UPD_STATUS = 'I' AND (p_search_GROUP_CD <> p_GROUP_CD OR p_search_USERID <> p_USERID OR P_CLIENT_CD<> P_SEARCH_CLIENT_CD) THEN
	...
	IF v_table_rowid IS NULL THEN
		BEGIN
			...
			FROM 
			(
				SELECT MAX(GROUP_CD) GROUP_CD, MAX(USERID) USERID, MAX(CLIENT_CD)CLIENT_CD
				FROM 
				(
					...
					AND d.field_name IN ('GROUP_CD' ,'USERID','CLIENT_CD')
	...
	IF  v_pending_cnt > 0 THEN
		v_error_code := -6;
		v_error_msg := 'Masih ada yang belum di-approve TAL '||p_search_GROUP_CD;
		RAISE v_err;
	END IF;
