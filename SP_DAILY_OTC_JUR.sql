...
IS
  ...
  CURSOR CSR_JUR
  IS
    SELECT T.TIDAK_DIJURNAL,T.CLIENT_CD, MAX(T.SUM_OTC)SUM_OTC, TRIM(BRANCH_CODE) BRANCH_CODE
    FROM T_DAILY_OTC_JUR T, MST_CLIENT M
    WHERE T.JUR_DATE  = P_OTC_DATE
    AND T.XN_DOC_NUM IS NULL
    AND T.CLIENT_CD = M.CLIENT_CD
    GROUP BY T.TIDAK_DIJURNAL,T.CLIENT_CD,TRIM(BRANCH_CODE)
    ORDER BY 1,2;
  ...
  V_BRANCH_CODE MST_BRANCH.BRCH_CD%TYPE;
BEGIN
  ...
  BEGIN
    SELECT DSTR1, DSTR2
    INTO V_GLA_BIAYA_YMH, V_SLA_BIAYA_YMH
    FROM MST_SYS_PARAM
    WHERE PARAM_ID='OTC_JOURNAL'
    AND PARAM_CD1 ='YMH';
  EXCEPTION
  WHEN OTHERS THEN
    V_ERROR_CD  :=-15;
  ...
  LOOP
        ...
        IF REC.TIDAK_DIJURNAL = 'N' THEN
            --AR/AP
            V_GL_ACCT_CD  := F_GL_ACCT_T3_JAN2016(REC.CLIENT_CD,V_DBCR_FLG);
            V_SL_ACCT_CD  := REC.CLIENT_CD;
            V_LEDGER_NAR  := 'BIAYA OTC '||REC.CLIENT_CD;
            V_ACCT_TYPE   := 'AR';
            V_BRANCH_CODE := REC.BRANCH_CODE;
        ELSE
            --5200
            V_LEDGER_NAR  := 'BIAYA OTC '||REC.CLIENT_CD ||' NONCHARGEABLE';
        ...
        BEGIN
          SP_T_ACCOUNT_LEDGER_UPD( V_JVCH_NUM,           --P_SEARCH_XN_DOC_NUM
          V_TAL_ID,                                      --P_SEARCH_TAL_ID
          V_JVCH_NUM,                                    --P_XN_DOC_NUM
          V_TAL_ID,                                      --P_TAL_ID
          V_DOC_REF_NUM,                                 --P_DOC_REF_NUM
          V_ACCT_TYPE, V_SL_ACCT_CD, V_GL_ACCT_CD, NULL, --    P_CHRG_CD,
          NULL,                                          --    P_CHQ_SNO
          'IDR',                                         --    P_CURR_CD
  ...
  IF V_TAL_ID > 0 THEN
        ...
        BEGIN
          SELECT NVL(SUM(SUM_OTC),0)
          INTO V_GRAND_TOTAL
          FROM T_DAILY_OTC_JUR A
          JOIN
            (
              SELECT MAX(ROWID) TABLE_ROWID
        ...
        BEGIN
          ...
          V_TAL_ID,                               --P_TAL_ID
          V_DOC_REF_NUM,                          --P_DOC_REF_NUM
          NULL, V_SL_ACCT_CD, V_GL_ACCT_CD, NULL, --    P_CHRG_CD,
          NULL,                                   --    P_CHQ_SNO
          'IDR',                                  --    P_CURR_CD
          NULL,                                   --    P_BRCH_CD
          ...
          NULL,                                   --   P_NETTING_DATE
          NULL,                                   --  P_NETTING_FLG
          V_RECORD_SOURCE, NULL,                  --   P_SETT_FOR_CURR
          NULL,                                   --  P_SETT_STATUS
          NULL,                                   --      P_RVPV_NUMBER
          V_FOLDER_CD,                            --    P_FOLDER_CD
          NULL,                                   --     P_SETT_VAL
        ...
        BEGIN
          SP_T_JVCHH_UPD ( V_JVCH_NUM, --P_SEARCH_JVCH_NUM
          V_JVCH_NUM,                  --    T_JVCHH.JVCH_NUM%TYPE,
        ...
        IF V_CEK_FOLDER='Y' THEN
                ...
                BEGIN
                  SP_T_FOLDER_UPD ( V_JVCH_NUM,         --P_SEARCH_DOC_NUM
                  TO_CHAR(V_JVCH_DATE,'MMYY'),          --P_FLD_MON
                  V_FOLDER_CD, V_JVCH_DATE, V_JVCH_NUM, --          T_FOLDER.DOC_NUM%TYPE,
                  P_USER_ID, SYSDATE,                   --P_CRE_DT
                  NULL,                                 -- P_UPD_BY
                  NULL,                                 -- P_UPD_DT
                  'I',                                  --P_UPD_STATUS
                  P_IP_ADDRESS, NULL,                   --P_CANCEL_REASON
                  V_UPDATE_DATE, V_UPDATE_SEQ, 1,       --P_RECORD_SEQ,
                  V_ERROR_CD,                           --P_ERROR_CODE,
                  V_ERROR_MSG                           --P_ERROR_MSG
                  );
                EXCEPTION
