...
IS
  ...
  V_ERR           EXCEPTION;
  V_CLIENT_TYPE_1 VARCHAR2(1);
  -- Edited by RD 26 Juli 2017 MST_PARAM 'BANKCD' to MST_IP_BANK
BEGIN
  ...
  BEGIN
     ...
     SELECT   DECODE(A.SUSP_STAT, 'N', 'Active', 'C', 'Closed', 'Suspend') status, INITCAP(CL2.CL_DESC) CL_DESC_2, INITCAP(CL3.CL_DESC) CL_DESC_3, F_SUBREK(S.SUBREK001)
        ...
        MARITL,(
            SELECT ip.BANK_CD,
              NVL(bi.BANK_NAME,ip.BANK_NAME) BANK_NAME
            FROM MST_IP_BANK ip,
              mst_bank_bi bi
            WHERE ip.BI_CODE = bi.BI_CODE(+)
            AND ip.APPROVED_STAT='A'
            AND bi.APPROVED_STAT(+)  ='A'
        )
        BANK,
        ...
        MST_SALES SALES, MST_BRANCH, MST_CIF CIF, V_CLIENT_SUBREK14 s, 
        (SELECT CLIENT_CD,BANK_ACCT_FMT FROM MST_CLIENT_FLACCT WHERE APPROVED_STAT='A') RDI,
        MST_CLIENT_INDI INDI, (SELECT BANK_CD,BANK_ACCT_NUM,ACCT_NAME,CIFS,CLIENT_cD FROM MST_CLIENT_BANK WHERE APPROVED_STAT='A') CLIENT_BANK,
  ...
  BEGIN
  SELECT CLIENT_TYPE_1 INTO V_CLIENT_TYPE_1 FROM MST_CLIENT WHERE CLIENT_CD=P_CLIENT_CD;
  exception      
    when no_data_found then
       V_ERROR_CD  := - 12;
        V_ERROR_MSG := SUBSTR('CLIENT_CD  not found '||V_ERROR_MSG||SQLERRM(SQLCODE),1,200);
        RAISE V_err;
    END;
  --INSERT DATA DARI MST_CLIENT_AUTHO USING CIFS
  BEGIN
  IF V_CLIENT_TYPE_1 = 'H' OR V_CLIENT_TYPE_1 = 'C' THEN
    INSERT INTO R_CLIENT_PROFILE_AUTHO
    (CIFS,FIRST_NAME, MIDDLE_NAME, LAST_NAME, POSITION, SEQNO, USER_ID, RAND_VALUE, GENERATE_DATE)
    SELECT A.CIFS, B.FIRST_NAME, B.MIDDLE_NAME, B.LAST_NAME, B.POSITION, B.SEQNO, P_USER_ID, V_RANDOM_VALUE, P_GENERATE_DATE
    FROM
    MST_CLIENT A,MST_CLIENT_AUTHO B
    WHERE A.CIFS=B.CIFS
    AND A.APPROVED_STAT='A'
  ...
  WHEN OTHERS THEN
    V_ERROR_CD  := -30;
    V_ERROR_MSG := SUBSTR('INSERT R_CLIENT_PROFILE_AUTHO '||SQLERRM(SQLCODE), 1, 200) ;
    RAISE V_err;
  END;
