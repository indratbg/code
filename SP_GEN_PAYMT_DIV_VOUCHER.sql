create or replace PROCEDURE SP_GEN_PAYMT_DIV_VOUCHER(
    P_PAYREC_DATE DATE,
...
-- Edited by RD 27 Juli 2017 change MST_PARAM "BANKCD" to MST_IP_BANK

  V_MENU_NAME T_MANY_HEADER.MENU_NAME%TYPE:='GENERATE PAYMENT DIVIDEN VOUCHER';
  ...
  IS
    SELECT b.TAL_ID,
      ...
      ||e.stk_cd
      ||' - '
      ||B.SL_ACCT_CD REMARKS,
      B.CURR_VAL AS curr_amt,
      f.bank_short_name rdi_bank_cd,
      'DIV '
      || E.stk_cd
      ||' '
      ||trim(TO_CHAR(E.qty,'9,999,999,999,999,999'))
      ||' @'
      ||E.RATE AS LEDGER_NAR,
      C.OLT
    FROM T_PAYRECH A,
      T_ACCOUNT_LEDGER B,
      MST_CLIENT C,
    ...
    AND E.DISTRIB_DT    = P_PAYREC_DATE
  ...
  IS
    SELECT DISTINCT BRCH_CD,
      ...
      GL_ACCT_CD,
      SL_ACCT_CD,
      SUM(CURR_AMT) OVER (PARTITION BY BRCH_CD,CLIENT_CD) AS TOTAL,
...
BEGIN
  ...
  LOOP
    ...
    BEGIN
      ...
      WHERE PARAM_ID='GEN_PAYMENT_VOC_DIV'
      AND PARAM_CD1 ='ROUND'
      AND PARAM_CD2 = TRIM(REC.BRCH_CD);
    EXCEPTION
    WHEN OTHERS THEN
      V_ERROR_CODE := -170;
      V_ERROR_MSG  := SUBSTR('SP_T_MANY_HEADER_INSERT '|| SQLERRM(SQLCODE),1,200);
