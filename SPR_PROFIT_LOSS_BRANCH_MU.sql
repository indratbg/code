create or replace 
PROCEDURE SPR_PROFIT_LOSS_BRANCH(
    P_BGN_DATE      DATE,
    P_END_DATE      DATE,
...
INSERT INTO TEMP_CTR_PL_BRANCH
...
INSERT INTO TEMP_TAL_PL_BRANCH 
SELECT TRIM(gl_acct_cd)
          ...
          AND T_ACCOUNT_LEDGER.GL_ACCT_CD IN( SELECT DISTINCT gl_A_cd FROM V_LABARUGI_ACCT_JAN2017)
    ...
    BEGIN
      ...
      FROM
        ...
        FROM
          ...
          FROM
            (
            SELECT * FROM TEMP_CTR_PL_BRANCH
            UNION ALL
            SELECT branch_Cd,
            ...
            FROM
              ...
              and B.APPROVED_STS = 'A'
               --[IN] 28 FEB 2017 UNTUK MINFEE
              union all
                select T.XN_DOC_NUM ,
                CURR_VAL,
                DB_CR_FLG,
                DOC_DATE,
                NVL(BRCH_CD,B.BRANCH_CODE)BRCH_CD
                from T_ACCOUNT_LEDGER T,
                (select XN_DOC_NUM,M.BRANCH_CODE from T_ACCOUNT_LEDGER T, MST_CLIENT M where T.SL_ACCT_CD=M.CLIENT_CD
                and DOC_DATE between P_BGN_DATE and P_END_DATE
                AND APPROVED_STS='A'
                )B
                where T.XN_DOC_NUM=B.XN_DOC_NUM
                AND DOC_DATE between P_BGN_DATE and P_END_DATE
                and TRIM(T.GL_ACCT_CD)=V_GLA_KOMISI
                and (SUBSTR(T.XN_DOC_NUM,5,3)='GLA' or SUBSTR(T.XN_DOC_NUM,5,6)='GLAMFE')
                and APPROVED_STS='A'
              --END MINFEE--
              )
            )
        ...
        SELECT v.gl_acct_group,
          ...
          V.LR_FAKTOR
        FROM
          ( SELECT * FROM TEMP_TAL_PL_BRANCH
          ) A,
          V_LABARUGI_ACCT_JAN2017 v,
          ( SELECT brch_cd ,BRCH_NAME, ACCT_PREFIX FROM mst_branch
          UNION
          SELECT 'ZI' BRCH_CD, 'FIXED INCOME' BRCH_NAME ,'90' ACCT_PREFIX FROM DUAL
          ) MST_BRANCH
...
SELECT TRIM(T_ACCOUNT_LEDGER.GL_ACCT_CD)
          ...
          AND T_ACCOUNT_LEDGER.GL_ACCT_CD    IN
            ( SELECT DISTINCT GL_A_CD FROM V_LABARUGI_ACCT_JAN2017
            )
    ...
    BEGIN
      ...
      FROM
          ...
          FROM
            ...
            FROM
              ...
              FROM
                ...
                T_ACCOUNT_LEDGER B
              where a.CONTR_NUM = B.XN_DOC_NUM
              AND trim(B.GL_ACCT_CD)  = V_GLA_KOMISI
              AND B.DOC_DATE BETWEEN P_BGN_DATE AND P_END_DATE
              and B.APPROVED_STS = 'A'
              --[IN] 28 FEB 2017 UNTUK MINFEE
              UNION ALL
              select XN_DOC_NUM ,
                ...
                and T.GL_ACCT_CD=V_GLA_KOMISI
                and (SUBSTR(XN_DOC_NUM,5,3)='GLA' or SUBSTR(XN_DOC_NUM,5,6)='GLAMFE')
                and APPROVED_STS='A'
        ...
        SELECT V.GL_ACCT_GROUP,
          ...
          V.LR_FAKTOR
        FROM
          (SELECT * FROM TEMP_TAL_PL_BRANCH) A,
          (SELECT GL_ACCT_GROUP,
            GL_ACCT_GROUP_NAME,
            GL_A,
            MAIN_ACCT_NAME,
            ...
            FAKTOR,
            LR_FAKTOR
          FROM V_LABARUGI_ACCT_JAN2017
          where GL_A like '5%'
          --AND ACCT_PREFIX = '10'--28FEB2017, MU PAKE SUBSTR(SL_ACCT_CD,1,1) 
          AND ACCT_PREFIX = '1'
          UNION
