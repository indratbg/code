...
IS
  ...
  NOTES:
  21 MAR 2017 - Ubah PEMBULATAN QTY diambil dari T_CORP_ACT
  ...
  ******************************************************************************/
  ...
  IS
     SELECT   CA_TYPE, p_stk_cd, CUM_DT, X_DT, RECORDING_DT, DISTRIB_DT, CLIENT_CD, CLIENT_NAME, BRANCH_CODE, CLIENT_TYPE, 
     FROM_QTY, TO_QTY, BEGIN_QTY, RECV_QTY,  DECODE(CA_TYPE,'STKDIV',BEGIN_QTY+RECV_QTY,END_QTY) END_QTY, ROUNDING, ROUND_POINT
         FROM
        (
           SELECT   A.CLIENT_CD, A.STK_CD, CA_TYPE, FROM_QTY, TO_QTY, CLIENT_TYPE, BEGIN_QTY, 
                DECODE(C.CA_TYPE,'STKDIV',(A.BEGIN_QTY * TO_QTY / FROM_QTY) - DECODE(C.TAX_FLG,'Y',(A.BEGIN_QTY * TO_QTY / FROM_QTY * M.TAX_PCN),0),
                A.BEGIN_QTY * TO_QTY / FROM_QTY)RECV_QTY,--21MAR2017
                DECODE(C.CA_TYPE, 'SPLIT', 0, 'REVERSE', 0, A.BEGIN_QTY) + (A.BEGIN_QTY * TO_QTY / FROM_QTY) END_QTY,--21MAR2017
                M.CLIENT_NAME, M.BRANCH_CODE, C.DISTRIB_DT, C.X_DT, C.RECORDING_DT, C.CUM_DT, C.ROUNDING, NVL(C.ROUND_POINT,0)ROUND_POINT
               FROM
              (
                ...
                FROM
                  (
                    SELECT M.CLIENT_CD, M.CLIENT_NAME, DECODE(A.RATE_OVER25PERSEN, NULL, DECODE(M.RATE_NO, 2, R.RATE_2, 1, R.RATE_1, 0), A.RATE_OVER25PERSEN) TAX_RATE, CLIENT_TYPE, BRANCH_CODE
                      FROM
                        (
                          SELECT CLIENT_CD, MST_CLIENT.CLIENT_NAME, 
                          ...
                          MST_CLIENT.BRANCH_CODE, DECODE(MST_CLIENT.CLIENT_CD, C.COY_CLIENT_CD, 'H', DECODE(MST_CLIENT.CLIENT_TYPE_1, 'H', 'H', '%')) AS CLIENT_TYPE
              ...
              (--25JAN2017, UNTUK CEK KAPAN MENGGUNAKAN PEMOTONGAN PAJAK DAN TIDAK
                SELECT DECODE(CA_TYPE, 'RIGHT', A_STK_CD, 'WARRANT', A_STK_CD, A.STK_CD) STK_CD, CA_TYPE, FROM_QTY, TO_QTY, 
                    DISTRIB_DT, X_DT, RECORDING_DT, CUM_DT, DECODE(B.STK_CD,NULL,'Y','N')TAX_FLG, A.ROUNDING, A.ROUND_POINT                 
                     FROM T_CORP_ACT A,
  ...
  BEGIN
    ...
    LOOP
            ...
            --21MAR2017
            IF REC.ROUNDING ='FLOOR' THEN
              V_RECV_QTY :=FLOOR(REC.RECV_QTY);
              V_BEGIN_QTY :=FLOOR(REC.BEGIN_QTY);
              V_END_QTY :=FLOOR(REC.END_QTY);
            ELSIF REC.ROUNDING ='ROUND' THEN
              V_RECV_QTY :=ROUND(REC.RECV_QTY,REC.ROUND_POINT);
              V_BEGIN_QTY :=ROUND(REC.BEGIN_QTY,REC.ROUND_POINT);
              V_END_QTY :=ROUND(REC.END_QTY,REC.ROUND_POINT);
            ELSE
              V_RECV_QTY :=CEIL(REC.RECV_QTY);
              V_BEGIN_QTY :=CEIL(REC.BEGIN_QTY);
              V_END_QTY :=CEIL(REC.END_QTY);
            END IF;
      
            IF P_Ca_type    = 'REVERSE' THEN
              V_QTY        := V_BEGIN_QTY - V_END_QTY;
            ELSIF P_Ca_type = 'SPLIT' THEN
              V_QTY        := V_END_QTY - V_BEGIN_QTY;
            ELSE
              V_QTY := V_RECV_QTY;
            END IF;
          ...
          IF V_QTY>0 THEN 
            ...
            IF P_CA_TYPE = 'MERGE' THEN
              ...
              BEGIN
                 SELECT   AVG_BUY_PRICE * V_BEGIN_QTY / V_RECV_QTY
                     INTO V_PRICE
                     FROM t_avg_price A,(
            ...
            FOR J IN 1..2  LOOP
              ...
              IF(J = 1) OR(J = 2 AND P_CA_TYPE = 'MERGE') THEN
                ...
                IF J = 2 THEN
                  ...
                  V_DOC_TYPE                               := 'WSN';
                  V_TOTAL_SHARE_QTY                        := 0;
                  V_WITHDRAWN_SHARE_QTY                    := V_BEGIN_QTY;
