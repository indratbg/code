create or replace PROCEDURE SP_T_PAYRECH_APPROVE
(
...
/*******************************************************************************
--[INDRA] 14 JUN 2017 TAMBAH CANCEL CASH TRANSACTION
--25JAN2017 CANCEL T_FUND_KSEI_LEDGER JUGA JIKA CANCEL VOUCHER KSEI
...
--			HAVING MAX(DOC_REF_NUM) <> MAX(PAYREC_NUM)
	...
	vi_trx_date DATE;--14 jun2017
	vi_trx_seq_no NUMBER(10);--14 jun2017
  v_doc_num t_fund_movement.doc_ref_num2%type;
BEGIN
	BEGIN
	...
	--	END IF;
		...
		IF v_acct_type = 'KSEI' THEN
			...
			END;
			 ...
			 --25JAN2017 CANCEL T_FUND_KSEI_LEDGER JUGA
	      BEGIN
	      UPDATE T_FUND_KSEI_LEDGER A SET APPROVED_STS='C' WHERE  EXISTS 
	      (SELECT DOC_NUM FROM T_FUND_KSEI B WHERE DOC_REF_NUM=v_payrec_num AND B.DOC_NUM =A.DOC_NUM);
				EXCEPTION
					WHEN OTHERS THEN
						v_error_code := -94;
					v_error_msg := SUBSTR('UPDATE T_FUND_KSEI_LEDGER '||SQLERRM,1,200);
					RAISE v_err;
			END;
       		--END 25JAN2017 CANCEL T_FUND_KSEI_LEDGER JUGA

		END IF;
    -----=================BEGIN CANCEL CASH TRANSACTION=====================
    --14 jun2017 cancel cash transaction
    IF trim(V_ACCT_TYPE)='ROR' THEN
        
        BEGIN
        SELECT DOC_REF_NUM2 INTO v_doc_num FROM T_FUND_MOVEMENT WHERE DOC_REF_NUM =v_payrec_num;
        EXCEPTION
         WHEN NO_DATA_FOUND THEN
          v_error_code := -95;
          v_error_msg := 'No data found doc ref num '||v_payrec_num||' in T_FUND_MOVEMENT';
          RAISE v_err;
					WHEN OTHERS THEN
						v_error_code := -94;
					v_error_msg := SUBSTR('SELECT DOC_REF_NUM2 FROM T_FUND_MOVEMENT '||SQLERRM,1,200);
					RAISE v_err;
			END;
      
    		BEGIN
    		SELECT TRX_DATE, TRX_SEQ_NO INTO vi_trx_date, vi_trx_seq_no FROM CASH_TRANSACTION WHERE TRX_STATUS='N' AND TRX_DOC_REF_NUM=v_doc_num ;
			EXCEPTION
      WHEN NO_DATA_FOUND THEN
        v_error_code := -95;
				v_error_msg := 'No data found doc num '||v_doc_num||' in CASH_TRANSACTION';
				RAISE v_err;
			WHEN OTHERS THEN
				v_error_code := -96;
				v_error_msg := SUBSTR('SELECT TRX DATE, TRX_SEQ_NO FROM CASH_TRANSACTION '||SQLERRM,1,200);
				RAISE v_err;
			END;

    		BEGIN
		     SP_PAYMENT_CANCEL (vi_trx_date,vi_trx_seq_no,p_approved_user_id ,v_error_code,v_error_msg);
			EXCEPTION
			WHEN OTHERS THEN
				v_error_code := -100;
				v_error_msg := SUBSTR('CALL SP_PAYMENT_CANCEL '||SQLERRM,1,200);
				RAISE v_err;
			END;
			IF v_error_code<0 THEN
				v_error_code := -101;
				v_error_msg := SUBSTR('CALL SP_PAYMENT_CANCEL '||v_error_msg,1,200);
				RAISE v_err;
			END IF;
    END IF;
    -----=================END CANCEL CASH TRANSACTION=====================
