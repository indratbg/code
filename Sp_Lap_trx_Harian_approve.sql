create or replace 
PROCEDURE Sp_Lap_trx_Harian_approve(
...
BEGIN
	...
	BEGIN
	SELECT FIELD_VALUE INTO V_TRX_DT FROM T_MANY_DETAIL WHERE UPDATE_SEQ= P_UPDATE_SEQ 
								AND UPDATE_DATE=P_UPDATE_DATE 
								AND TABLE_NAME='LAP_TRX_HARIAN' 
    ...
    */
	...
	BEGIN
	UPDATE LAP_TRX_HARIAN SET APPROVED_STS ='A',approved_by=p_approved_user_id, APPROVED_DT = SYSDATE WHERE update_seq=p_update_seq and update_date=p_update_date;
	EXCEPTION
			WHEN OTHERS THEN
				v_error_code := -3;
				v_error_msg :=  SUBSTR('UPDATE LAP_TRX_HARIAN '||SQLERRM,1,200);
				RAISE v_err;
		END;	
