  ...
  CREATE OR REPLACE FORCE VIEW "IPNEXTG"."V_IMP_NEW_REK_DANA" ("CLIENT_CD", "NEW_BANK_CD", "NEW_BANK_ACCT", "NAME", "BANK_NAME", "NEW_ACCT_FMT", "BANK_CD", "BANK_ACCT", "BANK_ACCT_FMT", "FLG", "BALANCE") AS 
  SELECT distinct x.client_Cd, x.bank_Cd new_bank_cd, x.rek_dana new_bank_acct, x.name, x.bank_name, x.bank_acct_fmt new_acct_fmt,
     y.bank_cd, y.bank_acct_num bank_acct, y.bank_acct_fmt, 'N' flg,nvl(f_fund_bal(x.client_Cd,trunc(sysdate)),0) balance
