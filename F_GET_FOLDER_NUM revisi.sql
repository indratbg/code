 ...
 --  v_fld_prefix := 'IJ-';
	...
	IF trim(p_prefix) = 'IJ-' THEN
		...
		(
			...
			AND SUBSTR(doc_num,5,3) in ('PDA','RDA')
			UNION
			SELECT folder_cd
			FROM
			(
				SELECT MAX(FOLDER_CD) FOLDER_CD, MAX(FLD_MON) FLD_MON, MAX(DOC_NUM) DOC_NUM
				FROM 
				(
					SELECT DECODE(field_name,'FOLDER_CD',field_value, NULL) FOLDER_CD,
							DECODE(field_name,'FLD_MON',field_value, NULL) FLD_MON,
					...
					AND FIELD_NAME IN ('FOLDER_CD','FLD_MON','DOC_NUM')
	...
	ELSE
		...
		(
			...
			FROM
			(
				SELECT MAX(FOLDER_CD) FOLDER_CD,  MAX(FLD_MON) FLD_MON
				FROM 
				(
					SELECT DECODE(field_name,'FOLDER_CD',field_value, NULL) FOLDER_CD,
							DECODE(field_name,'FLD_MON',field_value, NULL) FLD_MON,
					...
					AND a.UPDATE_DATE = b.UPDATE_DATE
					WHERE TABLE_NAME = 'T_FOLDER'
					AND FIELD_NAME IN ('FOLDER_CD','FLD_MON')
					AND APPROVED_STATUS = 'E'
				)
				GROUP BY UPDATE_DATE, UPDATE_SEQ, RECORD_SEQ
			)
