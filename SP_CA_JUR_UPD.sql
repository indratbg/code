...
FROM(
	...
	FROM(
        ...
        AND s_d_type NOT  IN  ('H','B','S','R'))
		 UNION ALL
	--	SELECT  CLIENT_CD, STK_CD, BEG_BAL_QTY, BEG_BAL_QTY--22aug2016
	...
	( SELECT CLIENT_CD,  DECODE(CLIENT_CD, C.COY_CLIENT_CD,'H', DECODE(CLIENT_TYPE_1,'H','H','%')) AS CLIENT_TYPE,CLIENT_NAME,BRANCH_CODE
	  ...
	  ( SELECT DECODE(CA_TYPE,'RIGHT',A_STK_CD,'WARRANT',A_STK_CD,stk_cd) STK_CD, CA_TYPE, FROM_QTY, TO_QTY, DISTRIB_DT,X_DT, RECORDING_DT, CUM_DT
...
BEGIN
	...
	END IF;
	 ...
	 BEGIN
		...
		BEGIN
		SELECT LOT_SIZE INTO V_LOT_SIZE FROM MST_COUNTER WHERE STK_CD = V_STK_CD;
		EXCEPTION
