create or replace 
PROCEDURE SPR_RECON_STK_BAL_MAIN_004(P_BGN_STK VARCHAR2,
...
V_ERR EXCEPTION;
 ...
 BEGIN
   ...
   v_random_value := abs(dbms_random.random);
    ...
    BEGIN
        SP_RPT_REMOVE_RAND('R_RECON_STK_BAL_MAIN_004',V_RANDOM_VALUE,V_ERROR_MSG,V_ERROR_CD);
    EXCEPTION
        WHEN OTHERS THEN
             V_ERROR_CD := -10;
	...
	SELECT '004' AS Main_rek,
  d.stk_cd,
  d.secu_end_bal,
  d.ksei,
  d.secu_end_bal - d.ksei AS selisih,
  P_DT_BGN_DATE,
  P_DT_END_DATE,
  P_USER_ID,
  P_GENERATE_DATE,
  ...
  FROM
    ...
    FROM
      (SELECT client_cd,
        stk_cd,
        gl_acct_cd,
        DECODE(trim(db_cr_flg),'D',1,-1) * (NVL(withdrawn_share_qty,0) + NVL(total_share_qty,0)) bal
      FROM t_stk_movement
    ...
    AND stk_Cd                <> 'IDR' --17nov2015
    AND stk_cd BETWEEN P_BGN_STK AND P_END_STK
    )
  GROUP BY stk_cd
  ) d
WHERE --d.secu_end_bal <> d.ksei
  ((d.secu_end_bal <> d.ksei
AND P_ALL_RECORD   = 'N')
OR P_ALL_RECORD    = 'Y' ); --17nov2015
...
P_ERROR_MSG := '';
 ...
 EXCEPTION
  ...
  WHEN OTHERS THEN
   ...
   RAISE;
END SPR_RECON_STK_BAL_MAIN_004;