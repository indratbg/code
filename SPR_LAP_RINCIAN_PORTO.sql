CREATE OR REPLACE PROCEDURE SPR_LAP_RINCIAN_PORTO(P_UPDATE_DATE DATE,
													P_UPDATE_SEQ NUMBER,
													P_END_DT DATE,
													P_BGN_DT DATE,
													P_USER_ID VARCHAR2,
...
SELECT P_UPDATE_DATE,P_UPDATE_SEQ,P_END_DT, s.rep_type, s.stk_Cd, NVL(p.price,0) price, 												
		s.port001 AS port001, s.port002, s.port004, s.client001, 0 client002,		
...
FROM( SELECT '1' rep_type,stk_Cd, SUM(end_ksei * main001 * portofolio_ab) port001,												
		...
		FROM( SELECT client_cd, stk_cd, 										
			   ...
			   FROM( SELECT client_cd, stk_cd, 									
				   ...
				   FROM(  SELECT client_cd, stk_cd, 0 beg_ksei,   								
						 ...
						 SELECT client_cd, stk_cd, 0 beg_ksei,   						
							  ...
							  (total_share_qty + withdrawn_share_qty),0)) ksei_mvmt					
						    FROM ipnextg.T_STK_MOVEMENT 						
						   WHERE doc_dt BETWEEN (P_END_DT - 10) AND P_END_DT						
									AND  due_dt_for_cert >P_END_DT			
							-- AND stk_cd >= :s_bgn_stk					
							 --AND stk_cd <= :s_end_stk					
							 AND gl_acct_cd = 'RR'					
							 AND gl_acct_cd IS NOT NULL					
			...
			( SELECT m.client_cd, 									
			  m.client_name, 									
			  agreement_no, 									
		             DECODE(m.client_cd,V_CLIENT_CD_AB, 1,0)  portofolio_ab,										
					    DECODE(m.client_cd,V_CLIENT_CD_AB, 0,DECODE(c.client_cd,NULL,1,0))  portofolio_client,							
					    DECODE(m.client_cd,V_CLIENT_CD_AB, 0,DECODE(c.client_cd,NULL,0,1))  client004,							
	   ...
	   FROM( SELECT DISTINCT agreement_no											
	          FROM( SELECT  m.client_cd, DECODE(s.subrek,NULL,m.agreement_no, s.subrek) agreement_no											
			          FROM( SELECT t.client_cd, t.old_str AS subrek									
									...
									AND ( P_END_DT > '31may2012' OR m.client_type_3 <> 'M')) s,			
           ...
           AND SUBSTR(NVL(agreement_no,'XXXXX0000004'),10,3) <> '004'												
			...
			SELECT subrek 									
				...
				WHERE t.UPD_DT > P_END_DT 								
				AND t.item_name = 'CLOSED' 								
				AND t.subrek_mvmt <> 0								
	         AND SUBSTR(t.subrek,10,3) = '001'											
            AND t.client_cd = m.client_cd 												
				AND ( P_END_DT > '31may2012' OR m.client_type_3 <> 'M')								
	         AND m.acct_open_dt <= P_END_DT											
			MINUS									
		   SELECT  t.new_str										
				FROM( SELECT client_cd, MIN(cre_dt) mindt 								
						FROM ipnextg.T_CLIENT_LOG 						
...
END IF;--END RINCIAN PORTOFOLIO YJ
