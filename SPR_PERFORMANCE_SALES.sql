...
AS
  ...
  IS
    ...
    FROM
      (SELECT TRIM(B.BRANCH_CODE) BRANCH_CODE,
        ...
        LAG( ROUND(B.CLIENT_COMM/ A.TOT_BRANCH,4), 1, 0) OVER (PARTITION BY TRIM(B.BRANCH_CODE) ORDER BY TRIM(B.BRANCH_CODE), B.CLIENT_COMM DESC) AS COMM_PRC_prev,
        SUM(ROUND(B.CLIENT_COMM / A.TOT_BRANCH,4)) OVER(PARTITION BY B.BRANCH_CODE) TOT_COMM_PRC
      FROM
        (SELECT TRIM(T.BRCH_CD) BRANCH_CODE,
          SUM(T.COMMISSION) TOT_BRANCH,
          COUNT(DISTINCT T.CLIENT_Cd) CNT
        FROM T_CONTRACTS T,
          MST_CLIENT M
    ...
    IF P_REPORT_MODE='DETAIL' THEN
      BEGIN
        ...
        FROM
          ...
          MST_SALES,
          MST_CLIENT
        WHERE TRIM(MST_BRANCH.BRCH_CD) = TRIM(T_CONTRACTS.BRCH_CD)
