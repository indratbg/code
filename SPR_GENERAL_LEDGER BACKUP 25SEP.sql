  ...
  IF P_MODE       ='REGULAR' THEN
    ...
    BEGIN
      ...
      FROM
        ...
        FROM
          ...
          FROM
            ...
            FROM T_ACCOUNT_LEDGER b,
              ...
              AND (TRIM(mst_gl_account.brch_cd) LIKE TRIM(as_bgn_branch)
            ...
            AND (m.jur_type      = 'NONTRX'
                OR ( m.jur_type      = 'TRX' AND a.record_source <> 'CG'  )) --17oct16
            ...
            FROM(               
                ...
                FROM
                  ...
                  AND TRIM(A.GL_ACCT_cD)   = TRIM(M.GL_A)
                  AND A.SL_ACCT_CD   = M.SL_A
                  AND (TRIM(m.brch_cd) LIKE TRIM(as_bgn_branch)
                  OR m.brch_Cd     IS NULL)
                  AND record_source = 'RE'
                  AND as_reversal   = 'Y'
                  ) a,
                  (SELECT a.xn_doc_num,
       ...
       --  AND NVL(m.acct_type,'123456') = 'LRTHIS' --31AUG2016
          AND m.sl_a BETWEEN as_bgn_sub AND as_end_sub
          AND ( TRIM(m.brch_cd) LIKE as_bgn_branch
            OR m.brch_Cd IS NULL)
          AND m.prt_type          <> 'S'
...
FROM
  ...
  FROM
    ...
    AND b.doc_date BETWEEN dt_bgn_date AND dt_end_date
    AND b.approved_sts <> 'C'
    AND b.approved_sts <> 'E'
    AND TRIM(B.GL_ACCT_cD)    = TRIM(MB.GL_A)
    AND B.SL_ACCT_CD    = MB.SL_A
    AND ( MB.brch_cd LIKE as_bgn_branch
      OR MB.brch_Cd IS NULL)
    AND mb.prt_type    <> 'S'
    GROUP BY b.doc_date,
      b.gl_acct_cd
    ) A1,
  ...
  WHERE M.GL_A BETWEEN TRIM(as_bgn_acct) AND TRIM(as_end_acct)
   AND ( m.brch_cd LIKE as_bgn_branch
      OR m.brch_Cd IS NULL)
  AND m.sl_a            = '000000'
