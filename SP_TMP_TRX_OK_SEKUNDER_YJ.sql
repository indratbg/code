create or replace PROCEDURE SP_TMP_TRX_OK_SEKUNDER_YJ(
    P_BGN_YEAR DATE,
    P_END_YEAR DATE,
    P_USER_ID    VARCHAR2,
  ...
  BEGIN
        ...
        BEGIN
        INSERT INTO TMP_TRX_NSB_OK_SEKUNDER(SEMESTER,GRP_1,GRP_2,GRP_3,NO_URUT,TRX_VALUE,RAND_VALUE,USER_ID)
        ...
        (
          ...
          DECODE(b.client_type_2,'L',1,2) no_urut,sum(cost) trx_val
          from 
            ...
            (
              select CLIENT_CD,CLIENT_TYPE_1,CLIENT_TYPE_2 from mst_client where client_type_1 <> 'B'
            ) b, 
            mst_bond e
            where a.sl_acct_cd = b.client_cd
            and a.bond_cd = e.bond_cd
            and e.bond_group_cd = '02'
          ...
          UNION ALL
          --BAGIAN KORPORASI DAN PERIKATAN LAINNYA
          SELECT SEMESTER,GRP_1,GRP_2,GRP_3,NO_URUT, SUM(TRX_VAL)TRX_VAL
          FROM(
                select decode(sign(to_number(to_char(trx_date,'mm')) - 6), 1, 2, 1) semester,
          ...
          FROM
            (
              SELECT DECODE(SIGN(to_number(TO_CHAR(trx_date,'mm')) - 6), 1, 2, 1) semester, 3 GRP_1, 
              ...
              FROM
                ...
                a, (
                  SELECT CLIENT_CD,CIFS,CLIENT_TYPE_1,CLIENT_TYPE_2
                  FROM mst_client
                  WHERE client_type_1 <> 'B'
                )
                b, mst_cif c, mst_bond e
              WHERE a.sl_acct_cd  = b.client_cd
              AND b.cifs          = c.cifs(+)
              AND a.bond_cd       = e.bond_cd
              AND e.bond_group_cd = '02'
      ...
      WHEN OTHERS THEN
        V_ERROR_CODE :=-32;
        V_ERROR_MSG :=SUBSTR('INSERT INTO TMP_TRX_NSB_OK_SEKUNDER'||SQLERRM,1,200);
        RAISE V_ERR;
      END;
    ...
    BEGIN
        ...
        SET SEKU_OBLIGASI_KORP=(SELECT TRX_VALUE FROM TMP_TRX_NSB_OK_SEKUNDER TMP WHERE RAND_VALUE=P_RANDOM_VALUE AND USER_ID=P_USER_ID
          AND TMP.SEMESTER= R.SEMESTER 
          AND TMP.GRP_1=R.GRP_1
          AND TMP.GRP_2 =R.GRP_2
          AND TMP.GRP_3=R.GRP_3
          AND TMP.NO_URUT = R.NO_URUT)
        WHERE EXISTS(SELECT 1 FROM TMP_TRX_NSB_OK_SEKUNDER TMP WHERE RAND_VALUE=P_RANDOM_VALUE AND USER_ID=P_USER_ID
          AND TMP.SEMESTER= R.SEMESTER 
          AND TMP.GRP_1=R.GRP_1
          AND TMP.GRP_2 =R.GRP_2
          AND TMP.GRP_3=R.GRP_3
    ...
    BEGIN
    DELETE FROM TMP_TRX_NSB_OK_SEKUNDER WHERE RAND_VALUE=P_RANDOM_VALUE AND USER_ID=P_USER_ID;
     EXCEPTION
      WHEN OTHERS THEN
        V_ERROR_CODE :=-40;
        V_ERROR_MSG  :=SUBSTR('DELETE FROM TMP_TRX_NSB_OK_SEKUNDER WHERE RAND_VALUE=P_RANDOM_VALUE'||SQLERRM,1,200);
        RAISE V_ERR;
      END;
