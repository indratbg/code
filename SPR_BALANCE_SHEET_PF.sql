CREATE OR REPLACE
PROCEDURE SPR_BALANCE_SHEET_PF(
    P_END_DATE      DATE,
    ...
    FROM
      ...
      FROM
        ...
        FROM
          ...
          FROM t_day_trs a,
            mst_gl_account g,
            ( SELECT gl_a FROM MST_GLA_TRX WHERE jur_type IN ('CLIE','T3','KPEI','T7')
            ) v
          WHERE a.trs_dt     = V_LAST_BAL_DATE
          AND (a.gl_acct_cd) = (g.gl_a)
          AND (a.sl_acct_cd) = (g.sl_a)
          AND TRIM(g.brch_cd) LIKE P_BRANCH_CD
          ...
          AND g.brch_cd LIKE P_BRANCH_CD
        ...
        SELECT trim(gl_a)
          ||'000000' AS gl_a,
          F_BALSH(item_type,'A','CURR',P_END_DATE,P_BRANCH_CD) curr_mvmt_A,
          F_BALSH(item_type,'A','LASTMON',P_END_DATE,P_BRANCH_CD ) last_mon_A,
          F_BALSH(item_type,'A','LASTYR',P_END_DATE,P_BRANCH_CD) prev_year_A,
          0 curr_mvmt_P,
          0 last_mon_P,
          0 prev_year_P
        FROM
          (SELECT gl_a,
            DECODE(jur_type,'CLIE','TRX','KPEI','KPEI','T3','ARAP35','T7','ARAP103') item_type
          FROM MST_GLA_TRX
          WHERE jur_type IN ( 'CLIED','KPEID','T3','T7')
            --and db_CR_flg = 'D'
          )
        UNION ALL
        SELECT DECODE(item_type,'ARAP159','1060',trim(gl_a))
          ||'000000' gl_a,
          0 curr_mvmt_A,
          0 last_mon_A,
          0 prev_year_A,
          F_BALSH(item_type,'P','CURR',P_END_DATE,P_BRANCH_CD) curr_mvmt_P,
          F_BALSH(item_type,'P','LASTMON',P_END_DATE,P_BRANCH_CD) last_mon_P,
          F_BALSH(item_type,'P','LASTYR',P_END_DATE,P_BRANCH_CD) prev_year_P
        FROM
          (SELECT gl_a,
            DECODE(jur_type,'CLIE','TRX','KPEI','KPEI','T3','ARAP159') item_type
          FROM MST_GLA_TRX
          WHERE jur_type IN ( 'CLIEC','KPEIC','T3')
