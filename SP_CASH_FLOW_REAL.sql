...
V_BGN_DATE DATE;
  ...
  --BROKER DAN KPEI
    BEGIN
    INSERT INTO TMP_CASH_FLOW_REAL(KATEGORI,MASUK,keluar,RAND_VALUE,USER_ID)
    SELECT b.sub_kategori kategori, DECODE(sub_kategori,'BJ',MASUK,'KJ',MASUK,0)MASUK, DECODE(sub_kategori,'BB',KELUAR,'KB',KELUAR,0)KELUAR, P_RAND_VALUE,P_USER_ID
    FROM
      (
        ...
        FROM
          (
            ...
            FROM
              (
                SELECT DECODE(A.ACCT_TYPE,'NEGO','B','K') KATEGORI,a.payrec_num, DECODE(b.db_cr_flg,'C',B.PAYREC_AMT,-B.PAYREC_AMT)NET_AMT
                FROM t_payrech a
                JOIN t_payrecd b
                ON a.payrec_num        =b.payrec_num
                WHERE a.APPROVED_STS   ='A'
                AND B.APPROVED_STS     ='A'
                AND TRIM(A.ACCT_TYPE) IN ('NEGO','KPEI')
                AND A.PAYREC_DATE      = P_REP_DATE
    ...
    END;
      ...
      --FIXED INCOME
       BEGIN
        INSERT INTO TMP_CASH_FLOW_REAL(KATEGORI,MASUK,keluar,RAND_VALUE,USER_ID)
          SELECT B.SUB_KATEGORI KATEGORI, DECODE(SUB_KATEGORI,'FJ',MASUK,0)MASUK, DECODE(SUB_KATEGORI,'FB',KELUAR,0)KELUAR, P_RAND_VALUE,P_USER_ID
          FROM(
              SELECT 'F' KATEGORI, SUM(DECODE(SIGN(NET_AMT),1,NET_AMT,0))MASUK, SUM(DECODE(SIGN(NET_AMT),1,ABS(NET_AMT),0))KELUAR
              FROM
                (
                  SELECT A.XN_DOC_NUM,SUM(DECODE(A.DB_CR_FLG,'D',CURR_VAL,-CURR_VAL)) NET_AMT
                  FROM T_ACCOUNT_LEDGER A
                  JOIN T_PAYRECH B
                  ON A.XN_DOC_NUM       =B.PAYREC_NUM
                  JOIN MST_GLA_TRX G
                  ON TRIM(A.GL_ACCT_cD)=G.GL_A
                  ...
                  AND TRIM(A.GL_ACCT_CD)=TRIM(B.GL_ACCT_CD)
                  AND A.SL_ACCT_cD      =B.SL_ACCT_CD
                  AND B.SL_ACCT_CD      ='300020'
                  GROUP BY A.XN_DOC_NUM
                )
            )
            A, (
          ...
          WHERE A.KATEGORI = B.KATEGORI;
          /*15FEB2018 WENNY MINTA AMBIL 1200 300020
  ...
  BEGIN
    ...
    --DEPOSITO + BANK GARANSI
        ...
        --ADP
        BEGIN
        SP_GENERATE_AGEING@YJFUND.ORCL(
            P_REP_DATE,
            P_USER_ID,
            '%',
            'ALL',
          ...
          EXCEPTION
            WHEN OTHERS THEN
            V_ERROR_CODE :=-62;
            V_ERROR_MSG :=SUBSTR('call SP_GENERATE_AGEING '||SQLERRM,1,200);
            RAISE V_ERR;
        END;
        
        IF V_ERROR_CODE<0 THEN
           V_ERROR_CODE :=-63;
            V_ERROR_MSG :=SUBSTR('call SP_GENERATE_AGEING '||V_ERROR_MSG,1,200);
            RAISE V_ERR;
        END IF;
  
         BEGIN
        INSERT INTO TMP_CASH_FLOW_REAL(KATEGORI,MASUK,keluar,RAND_VALUE,USER_ID)
        SELECT 'ADP', SUM(NVL(T0,0))MASUK,0,P_RAND_VALUE,P_USER_ID
        FROM RPT_AGEING@YJFUND.ORCL WHERE RAND_VALUE=VO_RANDOM_VALUE AND USER_ID=P_USER_ID AND T0>0;
        EXCEPTION
            WHEN OTHERS THEN
            V_ERROR_CODE :=-60;
        ...
        BEGIN
        	DELETE FROM RPT_AGEING@YJFUND.ORCL WHERE RAND_VALUE=VO_RANDOM_VALUE AND USER_ID=P_USER_ID ;
        EXCEPTION
            WHEN OTHERS THEN
            V_ERROR_CODE :=-70;
            V_ERROR_MSG :=SUBSTR('DELETE FROM RPT_AGEING@YJFUND.ORCL '||SQLERRM,1,200);
            RAISE V_ERR;
        END;
