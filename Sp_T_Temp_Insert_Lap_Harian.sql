create or replace 
PROCEDURE          Sp_T_Temp_Insert_Lap_Harian (
	   p_table_name 		   ipnextg.T_TEMP_HEADER.TABLE_NAME%TYPE,
	   p_table_rowid			ipnextg.T_TEMP_HEADER.TABLE_ROWID%TYPE,
	   p_status						  ipnextg.T_TEMP_HEADER.status%TYPE,
	   p_user_id             		ipnextg.T_TEMP_HEADER.user_id%TYPE,
	   p_ip_address            ipnextg.T_TEMP_HEADER.ip_address%TYPE,
	   p_cancel_reason	   ipnextg.T_TEMP_HEADER.cancel_reason%TYPE,   
	   p_temp_detail		IN OUT	 ipnextg.Types.TEMP_DETAIL_rc,
...
v_rec ipnextg.T_TEMP_DETAIL%ROWTYPE;
BEGIN
   v_update_seq := 0;

   BEGIN
   SELECT ipnextg.t_temp_Seq.NEXTVAL INTO v_update_seq  FROM dual;
   EXCEPTION
   WHEN OTHERS THEN
   			    v_error_code := -1;
...
VALUES ( v_update_Date, p_table_name , v_update_seq,
   ...
   WHEN OTHERS THEN
     ...
     ROLLBACK;
     RAISE;
END Sp_T_Temp_Insert_Lap_Harian;